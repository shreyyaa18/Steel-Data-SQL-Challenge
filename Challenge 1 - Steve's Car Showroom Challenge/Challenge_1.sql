...
CREATE TABLE sales (
    ...
    FOREIGN KEY (salesman_id)
        REFERENCES salespersons (salesman_id)
);
--------------------
...
FROM
    cars
        JOIN
    sales ON sales.car_id = cars.car_id
WHERE
...
SELECT 
    sp.name, COUNT(car_id) AS Number_of_cars
FROM
    salespersons sp
        JOIN
...
SELECT 
    sp.name, SUM(cost_$) AS Total_Revenue
FROM
    salespersons sp
        JOIN
    sales s ON s.salesman_id = sp.salesman_id
...
SELECT 
    cars.type, SUM(cars.cost_$) AS Total_revenue
FROM
    cars
GROUP BY cars.type;
--------------------
...
FROM
    ...
    cars c ON c.car_id = s.car_id
WHERE
    YEAR(purchase_date)='2022' AND c.style = 'SUV'
GROUP BY c.style;
--------------------

-- 9. What is the name and city of the salesperson who sold the most number of cars in the year 2023?
SELECT 
    sp.name, sp.city, COUNT(s.car_id) AS 'cars sold'
FROM
    salespersons sp
        JOIN
    sales s ON s.salesman_id = sp.salesman_id
...
SELECT 
    sp.name, sp.age, SUM(c.cost_$) AS HighestRevenue
FROM
    salespersons sp
        JOIN
    sales s ON s.salesman_id = sp.salesman_id
